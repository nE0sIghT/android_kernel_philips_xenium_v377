...
//added by pengwei 20150827 begin
#include <mach/mt_gpio.h>
//added by pengwei 20150827 end
...
//added by pengwei 20150827 begin
#define AW3216_CHARGER_CONTROL_PIN			(GPIO10 | 0x80000000)
#define AW3216_CHARGER_ENABLE_PIN			(GPIO16 | 0x80000000)
static kal_bool is_aw3216_enabled=KAL_FALSE;
static DEFINE_SPINLOCK(aw3216_drv_lock);
//added by pengwei 20150827 end
...
{
	//printk("---davidpeng,get_charging_setting_current,g_temp_CC_value=%d\n",g_temp_CC_value);
...
if(batt_cust_data.high_battery_voltage_support)
			g_jeita_recharging_voltage = 4280;//modified by wkp 20151010 4200 
else
...
kal_uint32 set_bat_charging_current_limit(int current_limit)
{
	battery_log(BAT_LOG_CRTI, "[BATTERY] set_bat_charging_current_limit (%d)\r\n",
			    current_limit);
	//printk("---davidpeng,set_bat_charging_current_limit,current_limit=%d\n",current_limit);
...
void set_bat_sw_cv_charging_current_limit(int current_limit)
{
	...
	//printk("---davidpeng,set_bat_sw_cv_charging_current_limit,current_limit=%d\n",current_limit);
...
void select_charging_curret(void)
{
	if (g_ftm_battery_flag) {
		...
		//printk("---davidpeng,select_charging_curret,g_temp_CC_value=%d\n",g_temp_CC_value);
...
static kal_uint32 charging_full_check(void)
{
	...
	//printk("---davidpeng,charging_full_check,\n");
...
static void pchr_sw_cv_charing_current_check(void)
{
	...
	//printk("---davidpeng,pchr_sw_cv_charing_current_check,\n");
...
static void pchr_turn_on_charging(void)
{
	...
	//printk("---davidpeng,pchr_turn_on_charging,\n");
...
PMU_STATUS BAT_PreChargeModeAction(void)
{
	...
	//printk("---davidpeng,BAT_PreChargeModeAction,\n");
	...
	//added by pengwei 20150827 begin
	spin_lock(&aw3216_drv_lock);
	if (is_aw3216_enabled)
	{
		mt_set_gpio_mode(AW3216_CHARGER_CONTROL_PIN, 0x00);
		mt_set_gpio_dir(AW3216_CHARGER_CONTROL_PIN, 0x01);
		mt_set_gpio_out(AW3216_CHARGER_CONTROL_PIN, 0x00);

		mt_set_gpio_mode(AW3216_CHARGER_ENABLE_PIN, 0x00);
		mt_set_gpio_dir(AW3216_CHARGER_ENABLE_PIN, 0x01);
		mt_set_gpio_out(AW3216_CHARGER_ENABLE_PIN, 0x01);
		
		is_aw3216_enabled = KAL_FALSE;
	}
	spin_unlock(&aw3216_drv_lock);
	//added by pengwei 20150827 end
...
PMU_STATUS BAT_ConstantCurrentModeAction(void)
{
	...
	//printk("---davidpeng,BAT_ConstantCurrentModeAction,\n");
	...
	//added by pengwei 20150827 begin
	spin_lock(&aw3216_drv_lock);
	if ((KAL_FALSE == is_aw3216_enabled)&&(STANDARD_CHARGER == BMT_status.charger_type))
	{
		mt_set_gpio_mode(AW3216_CHARGER_CONTROL_PIN, 0x00);
		mt_set_gpio_dir(AW3216_CHARGER_CONTROL_PIN, 0x01);
		mt_set_gpio_out(AW3216_CHARGER_CONTROL_PIN, 0x00);
		...
		mt_set_gpio_mode(AW3216_CHARGER_ENABLE_PIN, 0x00);
		mt_set_gpio_dir(AW3216_CHARGER_ENABLE_PIN, 0x01);
		mt_set_gpio_out(AW3216_CHARGER_ENABLE_PIN, 0x00);
		
		is_aw3216_enabled = KAL_TRUE;
	}
	spin_unlock(&aw3216_drv_lock);
	//added by pengwei 20150827 end
...
if(batt_cust_data.high_battery_voltage_support)
	...
	//printk("---davidpeng,BAT_TopOffModeAction,\n");
...
//modified by wkp 20151014 begin
	//added by pengwei 20150827 begin
	/*spin_lock(&aw3216_drv_lock);
	if (is_aw3216_enabled)
	{
...
PMU_STATUS BAT_BatteryFullAction(void)
{
	...
	//printk("---davidpeng,BAT_BatteryFullAction,\n");
...
PMU_STATUS BAT_BatteryHoldAction(void)
{
	...
	//printk("---davidpeng,BAT_BatteryHoldAction,\n");
...
PMU_STATUS BAT_BatteryStatusFailAction(void)
{
	...
	//printk("---davidpeng,BAT_BatteryStatusFailAction,\n");
