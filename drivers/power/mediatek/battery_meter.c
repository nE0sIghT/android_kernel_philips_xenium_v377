...
//added by pengwei 20160412 begin
#if 0
static int get_Ubvbat(void)
{
	char *ptr;
	int ubvbat1;
...
void sw_oam_init_v2(void)
{
	...
	//printk("davidpeng,sw_oam_init_v2,gFG_voltage=%d,gFG_capacity_by_v=%d,g_rtc_fg_soc=%d\n",gFG_voltage,gFG_capacity_by_v,g_rtc_fg_soc);

	ret = battery_meter_ctrl(BATTERY_METER_CMD_GET_BATTERY_PLUG_STATUS, &plugout_status);
	//printk("davidpeng,sw_oam_init_v2,plugout_status = %d\n",plugout_status);
	...
	if(plugout_status==0 && bat_is_charger_exist()==KAL_FALSE)
	{
		if(g_rtc_fg_soc==0)
		{
			...
			type=1;
			//printk("davidpeng,sw_oam_init_v2,TYPE = 1\n");
		}
		else
		{
			gFG_capacity_by_v=g_rtc_fg_soc;
			type=2;
			//printk("davidpeng,sw_oam_init_v2,TYPE = 2\n");
		}
	}
	else
	{
			...
			&& (abs(gFG_capacity_by_v-gFG_capacity_by_v_init)<abs(gFG_capacity_by_v_init-g_rtc_fg_soc)))
		{
			if(abs(gFG_capacity_by_v-gFG_capacity_by_v_init)>batt_meter_cust_data.cust_poweron_delta_hw_sw_ocv_capacity_tolrance)
			{
				...
				type=3;
				//printk("davidpeng,sw_oam_init_v2,TYPE = 3\n");
			}
			else
			{
				//use hw ocv;
				type=4;
				//printk("davidpeng,sw_oam_init_v2,TYPE = 4\n");
			}
		...
		else
		{
			if((abs(g_rtc_fg_soc-gFG_capacity_by_v_init)>batt_meter_cust_data.cust_poweron_delta_hw_sw_ocv_capacity_tolrance) || g_rtc_fg_soc==0)
			{
				gFG_capacity_by_v=gFG_capacity_by_v_init;
				type=5;
				//printk("davidpeng,sw_oam_init_v2,TYPE = 5\n");
			}
			else
			{
				gFG_capacity_by_v=g_rtc_fg_soc;
				type=6;
				//printk("davidpeng,sw_oam_init_v2,TYPE = 6\n");
			}
		}	
	}
...
void dod_init(void)
{
	...
	//printk("davidpeng,dod_init,g_rtc_fg_soc=%d\n",g_rtc_fg_soc);
	...
	{
		gFG_capacity_by_v = g_rtc_fg_soc;
		//printk("davidpeng,dod_init,set gFG_capacity_by_v to g_rtc_fg_soc,gFG_capacity_by_v=%d\n",gFG_capacity_by_v);
	}
#endif
#endif     
	...
	if (gFG_capacity_by_v == 0 && bat_is_charger_exist() == KAL_TRUE) {
		...
		//printk("davidpeng,dod_init,gFG_capacity_by_v==1\n");
	...
	if (batt_meter_cust_data.change_tracking_point)
	{
		...
		//printk("davidpeng,dod_init,change_tracking_point,gFG_15_vlot=%d\n",gFG_15_vlot);
	}
	else
	{
		...
		gFG_15_vlot = fgauge_read_v_by_capacity((100 - g_tracking_point));
		bm_print(BM_LOG_CRTI, "[FGADC] gFG_15_vlot = %dmV\n", gFG_15_vlot);
		//printk("davidpeng,dod_init,ELSE,gFG_15_vlot=%d\n",gFG_15_vlot);
...
//extern void Mt_Disable_AW3216_Charger_Function(unsigned char enable);
void oam_init(void)
{
	int ret = 0;
	kal_int32 vbat_capacity = 0;
	kal_bool charging_enable = KAL_FALSE;
	//int ubvbat=0;//added by pengwei 20160412
	...
	//added by pengwei 20151130 begin
	//Mt_Disable_AW3216_Charger_Function(1);
	//added by pengwei 20151130 end
	...
	//added by pengwei 20160412 begin
	#if 0
	bm_print(BM_LOG_CRTI, "[oam_init_inf] ubvbat=%d, gFG_voltage=%d, vol_bat=%d\n",ubvbat,gFG_voltage,g_booting_vbat);
	gFG_voltage = ubvbat;
	#endif
	//added by pengwei 20160412 end
	...
	//printk("davidpeng,oam_init,gFG_voltage=%d,gFG_capacity_by_v=%d,g_booting_vbat=%d,vbat_capacity=%d\n",gFG_voltage,gFG_capacity_by_v,g_booting_vbat,vbat_capacity);

	if (bat_is_charger_exist() == KAL_TRUE)
	{
		//added by pengwei 20151130 begin
		#if 0
		if (gFG_voltage > g_booting_vbat)
		{
			if (abs(gFG_voltage-g_booting_vbat)>120)
			{
				gFG_voltage -= 120;
			}
			else if (abs(gFG_voltage-g_booting_vbat)>90)
			{
				gFG_voltage -= 90;
			}
			else if (abs(gFG_voltage-g_booting_vbat)>60)
			{
				gFG_voltage -= 60;
			}
			else if (abs(gFG_voltage-g_booting_vbat)>30)
			{
				gFG_voltage -= 30;
			}
			else
		...
		{
			if (abs(g_booting_vbat-gFG_voltage)>120)
			{
				gFG_voltage = g_booting_vbat - 120;
			}
			else if (abs(g_booting_vbat-gFG_voltage)>90)
			{
				gFG_voltage = g_booting_vbat - 90;
			}
			else if (abs(g_booting_vbat-gFG_voltage)>60)
			{
				gFG_voltage = g_booting_vbat - 60;
			}
			else if (abs(g_booting_vbat-gFG_voltage)>30)
			{
				gFG_voltage = g_booting_vbat - 30;
			}
			else
			{
		...
		gFG_capacity_by_v = fgauge_read_capacity_by_v(gFG_voltage);
		printk("davidpeng,oam_init,ADJUST,gFG_capacity_by_v=%d\n",gFG_capacity_by_v);
		#endif
		//added by pengwei 20151130 end
			...
			batt_meter_cust_data.cust_poweron_delta_vbat_tolrance)) 
		{
			...
			//printk("davidpeng,oam_init, gFG_voltage using g_booting_vbat,gFG_voltage=%d,gFG_capacity_by_v=%d\n",gFG_voltage,gFG_capacity_by_v);
		}

		//added by pengwei 20160415 begin
		if (gFG_voltage < g_booting_vbat)
		{
			gFG_voltage = g_booting_vbat;
			gFG_capacity_by_v = vbat_capacity;

			//printk("davidpeng,oam_init, PENGWEI,gFG_voltage using g_booting_vbat,gFG_voltage=%d,gFG_capacity_by_v=%d\n",gFG_voltage,gFG_capacity_by_v);
		}
		//added by pengwei 20160415 end
	}
