...
//added by pengwei 20150818 begin
#ifdef CONFIG_ENABLE_REGULATOR_POWER_ON
static struct regulator *MsgregCTP = NULL;
static bool Msg_CTP_hwPowerOn(char * powerId, int powerVolt)
{
    bool ret = false;
	
	if(MsgregCTP == NULL)
		MsgregCTP = regulator_get(tpd->tpd_dev, powerId);
	
	//printk("[Msg_CTP_hwPowerOn]after get, powerId:%s, powerVolt:%d, MsgregCTP: %p\n", powerId, powerVolt, MsgregCTP);

 	if(!IS_ERR(MsgregCTP))
	{
        if(regulator_set_voltage(MsgregCTP , powerVolt,powerVolt)!=0 )
		{
            printk("[Msg_CTP_hwPowerOn]fail to regulator_set_voltage, powerVolt:%d, powerID: %s\n", powerVolt, powerId);
        }
        if(regulator_enable(MsgregCTP)!= 0) 
		{
        ...
        ret = true;
   		 ...
   		 printk("[Msg_CTP_hwPowerOn]IS_ERR_OR_NULL MsgregCTP %s\n",powerId);

    return ret;
}

static bool Msg_CTP_hwPowerDown(char * powerId)
{
    bool ret = false;
    ...
    if(!IS_ERR(MsgregCTP))
	{
        if(regulator_is_enabled(MsgregCTP)) 
		{
            printk("[Msg_CTP_hwPowerDown]before disable %s is enabled\n", powerId);
        }
		if(regulator_disable(MsgregCTP)!=0)
	...
	{
        printk("[Msg_CTP_hwPowerDown]%s fail to power down  due to MsgregCTP == NULL MsgregCTP 0x%p\n", powerId,MsgregCTP);
    }
	
    return ret;
}
#endif
//added by pengwei 20150818 end
...
void DrvPlatformLyrTouchDeviceRegulatorPowerOn(bool nFlag)
{
    ...
    Msg_CTP_hwPowerOn(PMIC_APP_CAP_TOUCH_VDD, 2800000);
#endif //CONFIG_PLATFORM_USE_ANDROID_SDK_6_UPWARD
...
//            enable_irq(_gIrq);
		...
		//modified by pengwei 20160316 begin
        ...
        mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
		//modified by pengwei 20160316 end
