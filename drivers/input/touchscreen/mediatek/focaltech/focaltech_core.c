...
* Return: 0
***********************************************************************/
//added by pengwei 20151013 begin
static struct regulator *FocalregCTP = NULL;
static bool Focal_CTP_hwPowerOn(char * powerId, int powerVolt)
{
	bool ret = false;
	
	if(FocalregCTP == NULL)
		FocalregCTP = regulator_get(tpd->tpd_dev, powerId);
	
	//printk("[Msg_CTP_hwPowerOn]after get, powerId:%s, powerVolt:%d, FocalregCTP: %p\n", powerId, powerVolt, FocalregCTP);

	if(!IS_ERR(FocalregCTP))
	{
		if(regulator_set_voltage(FocalregCTP , powerVolt,powerVolt)!=0 )
		{
			printk("[Msg_CTP_hwPowerOn]fail to regulator_set_voltage, powerVolt:%d, powerID: %s\n", powerVolt, powerId);
		}
		if(regulator_enable(FocalregCTP)!= 0) 
		{
			printk("[Msg_CTP_hwPowerOn]fail to regulator_enable, powerVolt:%d, powerID: %s\n", powerVolt, powerId);
			return ret;
		}
		ret = true;
	}
	else
		 printk("[Msg_CTP_hwPowerOn]IS_ERR_OR_NULL FocalregCTP %s\n",powerId);

	return ret;
}

static bool Focal_CTP_hwPowerDown(char * powerId)
{
	bool ret = false;
	...
	if(!IS_ERR(FocalregCTP))
	{
		if(regulator_is_enabled(FocalregCTP)) 
		{
			printk("[Msg_CTP_hwPowerDown]before disable %s is enabled\n", powerId);
		}
		if(regulator_disable(FocalregCTP)!=0)
			 printk("[Msg_CTP_hwPowerDown]fail to regulator_disable, powerID: %s\n", powerId);
		
		ret = true;
	} 
	else 
	{
		printk("[Msg_CTP_hwPowerDown]%s fail to power down	due to FocalregCTP == NULL FocalregCTP 0x%p\n", powerId,FocalregCTP);
	}
	
	return ret;
}
//added by pengwei 20151013 end
 ...
 {	 
	...
	//power on, need confirm with SA
		...
		if (TPD_POWER_SOURCE_CUSTOM!=MT65XX_POWER_NONE)
		{
			//hwPowerOn(TPD_POWER_SOURCE_CUSTOM, VOL_2800, "TP");
			Focal_CTP_hwPowerOn(PMIC_APP_CAP_TOUCH_VDD, 2800000);
		}
	#else
	...
	fts_get_upgrade_array();
		...
		printk("*******************(spl)Enter CTP Auto Upgrade********************\n");
...
static int __init tpd_driver_init(void) 
{
        ...
        i2c_register_board_info(FT_I2C_NUMBER, &fts_i2c_tpd, 1);
