...
#ifndef MT6572
extern void mt_eint_set_hw_debounce(unsigned int gpio_pin, unsigned int ms);
...
#endif
static u8 gt9xx_sensor_id=0x00;
#if GTP_ESD_PROTECT
...
CHIP_TYPE_T gtp_chip_type = CHIP_TYPE_GT_UNKOWN;//CHIP_TYPE_GT9;
...
//added by pengwei 20150715 begin
static struct regulator *regCTP = NULL;
static bool CTP_hwPowerOn(char * powerId, int powerVolt)
{
    bool ret = false;
	
	if(regCTP == NULL)
		regCTP = regulator_get(tpd->tpd_dev, powerId);
	
	//printk("[CTP_hwPowerOn]after get, powerId:%s, powerVolt:%d, regCTP: %p\n", powerId, powerVolt, regCTP);

 	if(!IS_ERR(regCTP))
	{
        if(regulator_set_voltage(regCTP , powerVolt,powerVolt)!=0 )
		{
            printk("[CTP_hwPowerOn]fail to regulator_set_voltage, powerVolt:%d, powerID: %s\n", powerVolt, powerId);
        }
        if(regulator_enable(regCTP)!= 0) 
		{
            printk("[CTP_hwPowerOn]fail to regulator_enable, powerVolt:%d, powerID: %s\n", powerVolt, powerId);
            return ret;
        }
        ret = true;
    }
	else
   		 printk("[CTP_hwPowerOn]IS_ERR_OR_NULL regCTP %s\n",powerId);

    return ret;
}

static bool CTP_hwPowerDown(char * powerId)
{
    bool ret = false;
    ...
    if(!IS_ERR(regCTP))
	{
        if(regulator_is_enabled(regCTP)) 
		{
            printk("[CTP_hwPowerDown]before disable %s is enabled\n", powerId);
        }
		if(regulator_disable(regCTP)!=0)
			 printk("[CTP_hwPowerDown]fail to regulator_disable, powerID: %s\n", powerId);
		
        ret = true;
    } 
	else 
	{
        printk("[CTP_hwPowerDown]%s fail to power down  due to regCTP == NULL regCTP 0x%p\n", powerId,regCTP);
    }
...
s32 gtp_i2c_write(struct i2c_client *client, u8 *buf, s32 len)
{
	...
	#endif
	//modified by pengwei 20150716 end
...
static s8 gtp_i2c_test(struct i2c_client *client)
{
    ...
    while (retry++ < 5)
    {
		...
		//printk("davidpeng, gtp_i2c_test, ret=%d, hw_info=0x%x\n",ret,hw_info);
...
reset_proc:
	...
	CTP_hwPowerOn(TPD_POWER_SOURCE_CUSTOM, 2800000);
#endif
	...
	//added by pengwei 20140606 begin
	if (CHIP_TYPE_GT_UNKOWN==gtp_chip_type)
	{
		if (reset_count < 2)
		{
			reset_count++;
...
void gtp_get_chip_type(struct i2c_client *client)
{
    ...
    if (FAIL == ret)
    {
        ...
        gtp_chip_type = CHIP_TYPE_GT_UNKOWN;//CHIP_TYPE_GT9;
        return;
    }
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (ret < 0)
    {
        printk("davidpeng,tpd_i2c_probe,I2C communication ERROR!");
		return -1; //added by pengwei 20140606
    }
    
#if 0//def VELOCITY_CUSTOM
	...
	printk("davidpeng,tpd_i2c_probe, I2C probe OK!\n");
...
static void tpd_eint_interrupt_handler(void)
{
	...
	//printk("davidpeng, tpd_eint_interrupt_handler\n");
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        else 
        {
            ...
            else
            {
				...
				//modified by pengwei 20150716 begin
				#if 0//GTP_SUPPORT_I2C_DMA
    			i2c_write_bytes_dma(i2c_client_point, 0x8040, esd_buf, 1);
				#else
...
static int touch_event_handler(void *unused)
{
    ...
    do
    {
		...
		//printk("davidpeng, touch_event,gt9xx_sensor_id=%d\n",gt9xx_sensor_id);
...
static int tpd_local_init(void)
{
    ...
    gpDMABuf_va = (u8 *) dma_alloc_coherent(&tpd->dev->dev, 1024, &gpDMABuf_pa, GFP_KERNEL);
    if(!gpDMABuf_va){
...
#ifdef GT_I2C_BUS_NUMBER
    i2c_register_board_info(GT_I2C_BUS_NUMBER, &i2c_tpd, 1);
#else
