...
int disp_bls_set_backlight(int level_1024)
{
	...
	//printk("davidpeng,real,disp_bls_set_backlight,level=%d\n",level_1024);
	//added by pengwei 20151215 begin
	if (level_1024 > 0)
	{
		if (level_1024 > 1023)
			level_1024 = 1023;
		...
		if (level_1024 < 64)
			...
			new_level = level_1024;
	}
	//added by pengwei 20151215 end
    return disp_pwm_set_backlight(disp_pwm_get_main(), new_level);
}
...
int disp_pwm_set_backlight_cmdq(disp_pwm_id_t id, int level_1024, void *cmdq)
{
    ...
    if ((DISP_PWM_ALL & id) == 0) {
		//printk("davidpeng,disp_pwm_set_backlight_cmdq,error ID\n");
	...
	//printk("davidpeng,disp_pwm_set_backlight_cmdq,old_pwm=%d,level_1024=%d\n",old_pwm,level_1024);
