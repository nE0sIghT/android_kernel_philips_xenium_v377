...
//added by pengwei 20150831 begin
#include <linux/proc_fs.h>  //proc file use
//added by pengwei 20150831 end
...
//added by pengwei 20150829 begin
#define POWER_SAVE_KEY_MACRO_DEFINITION

#ifdef POWER_SAVE_KEY_MACRO_DEFINITION
#define CUST_EINT_POWER_SAVE_NUM              	1
#define CUST_EINT_POWER_SAVE_DEBOUNCE_CN		100
#define GPIO_EINT_POWER_SAVE_PIN				(GPIO1 | 0x80000000)
static volatile int hw_eint_power_save_key_status=0x00;
static volatile int hw_eint_power_save_key_polarity=0x00;
static void hw_eint_power_save_handler(void)
{
	if(!kpd_input_dev) 
	{
		printk("KPD input device not ready\n");
	...
	mt_eint_mask(CUST_EINT_POWER_SAVE_NUM);

	if (hw_eint_power_save_key_polarity)//power save STATUS is high
	{
		printk("------davidpeng, power save key is HIGH\n");
		hw_eint_power_save_key_status = 0x01;
		hw_eint_power_save_key_polarity = 0x00;

		//input_report_key(kpd_input_dev, KEY_PVR, 0);
		//input_sync(kpd_input_dev);
	...
	else//power save status is low
	{
		printk("------davidpeng, power save key, status is LOW\n");
		hw_eint_power_save_key_status = 0x00;
		hw_eint_power_save_key_polarity = 0x01;

		//input_report_key(kpd_input_dev, KEY_PVR, 1);
		//input_sync(kpd_input_dev);
		input_report_key(kpd_input_dev, KEY_F10, 1);		
		input_sync(kpd_input_dev);
	...
	{
		input_report_key(kpd_input_dev, KEY_F10, hw_eint_power_save_key_polarity);
		input_sync(kpd_input_dev);
	}
	else
	{
		//input_report_switch(kpd_input_dev, SW_LID, hw_eint_power_save_key_polarity);
		//input_report_key(kpd_input_dev, KEY_F10, hw_eint_power_save_key_polarity);		
		//input_sync(kpd_input_dev);
	...
	mt_eint_set_polarity(CUST_EINT_POWER_SAVE_NUM, hw_eint_power_save_key_polarity);
	mt_eint_unmask(CUST_EINT_POWER_SAVE_NUM);
	
}
#endif
//added by pengwei 20150829 end
...
#ifdef POWER_SAVE_KEY_MACRO_DEFINITION
	//POWER SAVE KEY
	mt_set_gpio_mode(GPIO_EINT_POWER_SAVE_PIN, GPIO_MODE_07);
	mt_set_gpio_dir(GPIO_EINT_POWER_SAVE_PIN, GPIO_DIR_IN);
	mt_set_gpio_pull_enable(GPIO_EINT_POWER_SAVE_PIN, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(GPIO_EINT_POWER_SAVE_PIN, GPIO_PULL_UP);

	gpio_val = mt_get_gpio_in(GPIO_EINT_POWER_SAVE_PIN);

	mt_eint_set_hw_debounce(CUST_EINT_POWER_SAVE_NUM, CUST_EINT_POWER_SAVE_DEBOUNCE_CN);
	if (gpio_val)
	{
		hw_eint_power_save_key_status = 0x01;
		hw_eint_power_save_key_polarity = 0x00;
		mt_eint_registration(CUST_EINT_POWER_SAVE_NUM, EINTF_TRIGGER_LOW, hw_eint_power_save_handler, 0);
	}
	else
	{
		hw_eint_power_save_key_status = 0x00;
		hw_eint_power_save_key_polarity = 0x01;
		mt_eint_registration(CUST_EINT_POWER_SAVE_NUM, EINTF_TRIGGER_HIGH, hw_eint_power_save_handler, 0);
	}
	mt_eint_unmask(CUST_EINT_POWER_SAVE_NUM);
#endif
//added by pengwei 20150829 end
...
static int vkpd_pwrsave_key_show(struct seq_file *m, void *v)
{
	int i;
	SEQ_printf(m, "%d\n", hw_eint_power_save_key_status);
	return 0;
}

static int vkpd_pwrsave_key_open(struct inode *inode, struct file *file)
{
	return single_open(file, vkpd_pwrsave_key_show, inode->i_private);
}

static const struct file_operations fvkpd_proc_pwrsave_key_fops = 
{
	.open = vkpd_pwrsave_key_open,
	.read = seq_read,
};
...
static int vkpd_proc_file()
{
	...
	dir = proc_mkdir("vkpd", NULL);
    proc_create("powersavekey", S_IRUGO, dir, &fvkpd_proc_pwrsave_key_fops);
}
//added by pengwei 20150831 end
...
static int __init kpd_mod_init(void)
{
	...
	vkpd_proc_file();//added by pengwei 20150831
