...
static imgsensor_info_struct imgsensor_info = { 
	...
	.checksum_value = 0x138daa55,		//checksum value for Camera Auto Test
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
			...
			{
				//2015-1-14 sp_miao
				printk("davidpeng, [SP8408]get_imgsensor_id, OK, sensor id is 0x%x\n",*sensor_id);  
				return ERROR_NONE;
			}
				
			printk("davidpeng,[SP8408]get_imgsensor_id,Read sensor id fail, id: 0x%x,0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
	...
	if (*sensor_id != imgsensor_info.sensor_id) 
	{
		...
		printk("davidpeng, [SP8408]get_imgsensor_id, ERROR\n");
   ...
   return ERROR_SENSOR_CONNECT_FAIL;
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
			if (sensor_id == imgsensor_info.sensor_id) {				
				printk("davidpeng,[SP8404],OPEN,i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);	  
				break;
			}	
			printk("davidpeng,[SP8404],OPEN,Read sensor id fail, id: 0x%x\n",sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
	...
	if (enable) {
		write_cmos_sensor(0x3221,0x01);
		write_cmos_sensor(0x3243,0x00);
		write_cmos_sensor(0x3245,0x00);
		write_cmos_sensor(0x339E,0x04);
		write_cmos_sensor(0x4B05,0x00);
		write_cmos_sensor(0x4B06,0x00);
		write_cmos_sensor(0x4B07,0x00);
		write_cmos_sensor(0x35F0,0x00);
		write_cmos_sensor(0x35F1,0x00);
		write_cmos_sensor(0x0601,0x02);
	} else {
		write_cmos_sensor(0x0601,0x00); // disable colorbar 
	}	 
	spin_lock(&imgsensor_drv_lock);
