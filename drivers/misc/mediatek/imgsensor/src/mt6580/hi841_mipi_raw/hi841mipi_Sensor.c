...
{
{ 3264, 2448,   16,  6, 3264, 2448, 3264, 2448, 0000, 0000, 3264, 2448,    2,  2, 3264, 2448}, // full preview
 //{ 3264, 2448,    8,  2, 2608, 1952, 1296,  972, 0000, 0000, 1296,  972,    2,  2, 1632,  1224}, // Preview
// { 3264, 2448,    0,    0, 3264, 2448,  3264, 2448, 0000, 0000, 1632, 1224,      0,    0, 1632, 1224},
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
			...
			do {
				*sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
				...
				LOG_INF("get_imgsensor_id Read sensor id fail, id: i2c_write_id = 0x%x, sensor_id=0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				retry--;
			} while(retry > 0);
			i++;
			retry = 2;
...
static kal_uint32 open(void)
{
	...
	LOG_INF("[open]: PLATFORM:MT6571,MIPI 4LANE\n");
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			LOG_INF("open:Read sensor id fail open w, id:  0x%x,0x%x\n", imgsensor.i2c_write_id,sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
