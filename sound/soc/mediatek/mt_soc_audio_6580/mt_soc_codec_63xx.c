...
#define EXT_AUDIO_PA_CONTROL_PIN			(GPIO21 | 0x80000000)

#define GAP (2) //unit: us
#define AW8736_MODE3 /*0.8w*/ \
    mt_set_gpio_out(EXT_AUDIO_PA_CONTROL_PIN,GPIO_OUT_ONE); \
    udelay(GAP); \
    mt_set_gpio_out(EXT_AUDIO_PA_CONTROL_PIN,GPIO_OUT_ZERO); \
    udelay(GAP); \
    mt_set_gpio_out(EXT_AUDIO_PA_CONTROL_PIN,GPIO_OUT_ONE); \
    udelay(GAP); \
    mt_set_gpio_out(EXT_AUDIO_PA_CONTROL_PIN,GPIO_OUT_ZERO); \
...
#define SPK_WARM_UP_TIME        (25) //unit is ms
    ...
    if (enable)
    {
        ...
        mt_set_gpio_mode(EXT_AUDIO_PA_CONTROL_PIN, GPIO_MODE_00); //GPIO117: DPI_D3, mode 0
        mt_set_gpio_pull_enable(EXT_AUDIO_PA_CONTROL_PIN, GPIO_PULL_ENABLE);
        mt_set_gpio_dir(EXT_AUDIO_PA_CONTROL_PIN, GPIO_DIR_OUT); // output
        mt_set_gpio_out(EXT_AUDIO_PA_CONTROL_PIN, GPIO_OUT_ZERO); // low disable
        udelay(2000);
        ...
        AW8736_MODE2;
#else
        mt_set_gpio_out(EXT_AUDIO_PA_CONTROL_PIN, GPIO_OUT_ONE); // high enable
#endif
    ...
    else
    {
        ...
        mt_set_gpio_mode(EXT_AUDIO_PA_CONTROL_PIN, GPIO_MODE_00); //GPIO117: DPI_D3, mode 0
        mt_set_gpio_dir(EXT_AUDIO_PA_CONTROL_PIN, GPIO_DIR_OUT); // output
        mt_set_gpio_out(EXT_AUDIO_PA_CONTROL_PIN, GPIO_OUT_ZERO); // low disbale
        udelay(2000);
#endif
        printk("Ext_Speaker_Amp_Change OFF- \n");
    }
